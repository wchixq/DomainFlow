...
from net.ellen_net import Deconv2Net, MLP2DropAleatoricClassifier
...
def train(args):
    ...
    exp_name = 'VAE%.2f_KLlatent%.2f_seed%d_'%(w_recon,w_latent,seed) + source[0] + '2' + target[0]
    ...
    # net -----------------------------------------------------------------------------
    encoder = resnet50backbone()
    decoder = Deconv2Net(channels=(256, 128, 3))
    classifier = MLP2DropAleatoricClassifier(channels=(2048, fc_dim, num_class))
    ...
    for epoch in range(1, num_epochs+1):
        ...
        for (img_src, label_s, _), (img_tgt, _, _) in zip(train_src_loader, train_tgt_loader):
            ...
            both_logit, both_mu, both_logvar = classifier.forward_train(out_both)
            ...
            l_latent = -0.5 * (1 + both_logvar - both_mu.pow(2) - both_logvar.exp()).mean()
            ...
            if it == num_batches:
                ...
                sigma_s = float(both_logvar[:batch_size].mean().item())
                sigma_t = float(both_logvar[batch_size:].mean().item())
                break
            it += 1
        encoder.eval()
        classifier.eval()
        ...
        with torch.no_grad():
            for image, label, _ in test_loader:
                ...
                output, _ = classifier(encoder(image))
        ...
        log_with_print(log_writer, 'epoch%03d: l_ce:%.4f l_recon:%.4f l_latent:%.4f sigma_s:%.3f sigma_t:%.3f test_acc:%.2f/%.2f in%.1fmin' % (
            epoch, l_ce_val, l_recon_val, l_latent_val, sigma_s, sigma_t, acc_ep, best_tgt_acc, time_taken))
        log_writer.flush()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weight_latent', type=float, default=0.1, metavar='TS', help='Domain ID')
