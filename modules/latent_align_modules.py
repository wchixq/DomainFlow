...
class FlowStep_fc(nn.Module):
	def __init__(self,in_channels, out_channels, hidden_channels,cond_dim,coupling):
		...
		# GLOW flow model
			...
			# permutation of features to allow correlated dimensions
...
class FlowLatent(nn.Module):
	...
	K: number of flow steps in one layer
	gaussian_var: if given, usually less than 1.0
	"""
	def __init__(self,batch_size,input_dim,hidden_channels=2048,K=16,gaussian_dims=(1024-700),gaussian_var=0.1,
		...
		self.relu = nn.ReLU()
		self.mean = torch.zeros(batch_size,self.input_dim, device='cuda')#.float().to(device)
		self.logs = torch.zeros(batch_size,self.input_dim, device='cuda')#).float().to(device)
		if gaussian_var != 0:
	...
	def normal_flow(self, x, z_im, cond=None):
		...
		if self.gaussian_dims > 0:
			objective = objective + GaussianDiag.logp(self.mean[:,-self.gaussian_dims:],
																 self.logs[:,-self.gaussian_dims:],
																 z[:,-self.gaussian_dims:].cuda())
		nll = (-objective) / float(np.log(2.)*x_shape[1])
		return z, nll, None

	def forward_flow(self, x, cond=None):
		x_shape = list(x.size())
		z = x
		logdet = torch.zeros_like(x[:, 0])
		logdet = logdet
	...
	def reverse_cond(self, cond):
		z = GaussianDiag.sample(self.mean, self.logs, eps_std=None)
		x = self.flow(z, cond=cond, eps_std=None, reverse=True)
		return x

	def reverse_uncond(self, x):
		if self.gaussian_dims != 0:
			z = GaussianDiag.sample(self.mean, self.logs, eps_std=None)
			z = torch.cat((x, z[:,-self.gaussian_dims:]), dim=1)
		else:
			z = x
		x = self.flow(z, cond=None, eps_std=None, reverse=True)
		return x
